...
import historyData from '../ssqData.json'
import {getSsqHistoryPageList} from "../api/common";
defineProps({
  width: Number,
  height: Number
})
...
function notAppear(area: string, times: number) {
  ...
  for (const key of arr) {
    let slice = []
    if (area === 'front') {
      slice = historyData[key].slice(0, 6)
    } else if (area === 'end') {
      slice = historyData[key].slice(6, 7)
    }
...
onMounted(async () => {
  // let historyNewData = {}
  // let historyList = historyData.result
  // for (const h of historyList) {
  //   const arr = h.front.split(',')
  //   arr.push(h.end)
  //   historyNewData[h.code] = arr
  // }
...
function prob() {
  ...
  for (const r of frontArr) {
    ...
    for (const s of frontSliceArr) {
      ...
      for (const key of keys.slice(len - s, len)) {
        if (historyData[key].slice(0, 6).indexOf(r) >= 0) {
          bs++
        }
      }
  ...
  for (const b of endArr) {
    ...
    for (const s of endSliceArr) {
      let bs = 0
      for (const key of keys.slice(len - s, len)) {
        if (historyData[key].slice(6, 7).indexOf(b) >= 0) {
          bs++
        }
      }
